...
function toRow(pkg, perf, { storeId, image, adAccountId, bumpState }) {
  ...
  return {
    ...
    governoratesTop3: gov.slice(0, 3).map((g) => ({ governorate: g.segment, orders: g.orders, confirmed: g.confirmed, delivered: g.delivered, status: g.status || g.classification || null })),
    bump: bumpState,
    eligibility,
...
/** One product's full Scale Center row — real reuse of getProductPerformance() (raw Meta/EasyOrders blocks) + buildProductDecisionPackage() (decision/dataQuality/CR/governorates) + an on-demand ad-set bump check for that product's own real campaigns. Two calls into the same underlying data on purpose: buildProductDecisionPackage()'s own diagnosis reshape drops fields (impressions) this page needs to show. */
